...
abstract contract BFactoryTest {
...
abstract contract ConfigurableRightsPoolTest is AbstractPool {
    struct PoolParams {
...
abstract contract CRPFactoryTest {
    function newCrp(
        address factoryAddress,
        ConfigurableRightsPoolTest.PoolParams calldata params,
        RightsManager.Rights calldata rights
    ) external virtual returns (ConfigurableRightsPoolTest);
}
...
contract BActions {
    ...
    function create(
        BFactoryTest factory,
        address[] calldata tokens,
        uint[] calldata balances,
        uint[] calldata weights,
        uint swapFee,
        bool finalize
    ) external returns (BPoolTest pool) {
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ERC20Test token = ERC20Test(tokens[i]);
    ...
    function createSmartPool(
        CRPFactoryTest factory,
        BFactoryTest bFactory,
        ConfigurableRightsPoolTest.PoolParams calldata poolParams,
        ConfigurableRightsPoolTest.CrpParams calldata crpParams,
        RightsManager.Rights calldata rights
    ) external returns (ConfigurableRightsPoolTest crp) {
        require(
        ...
        for (uint i = 0; i < poolParams.constituentTokens.length; i++) {
            ERC20Test token = ERC20Test(poolParams.constituentTokens[i]);
            require(
    ...
    function joinSmartPool(
        ConfigurableRightsPoolTest pool,
        uint poolAmountOut,
        uint[] calldata maxAmountsIn
    ) external {
    ...
    function finalize(BPoolTest pool) external {
        pool.finalize();
        require(pool.transfer(msg.sender, pool.balanceOf(address(this))), "ERR_TRANSFER_FAILED");
    }
    ...
    function increaseWeight(
        ConfigurableRightsPoolTest crp,
        ERC20Test token,
        uint newWeight,
        uint tokenAmountIn
    ) external {
    ...
    function migrateProportionally(
        ...
        uint lowestRatio = type(uint).max;
        uint lowestRatioToken = 0;
        for (uint i = 0; i < outTokens.length; ++i) {
            uint ratio = 1 ether * ERC20Test(outTokens[i]).balanceOf(address(this)) / tokenInAmounts[i];
        ...
        for (uint i = 0; i < outTokens.length; ++i) {
            ...
            if (lowestRatioToken == i) {
                tokenInAmounts[i] = ERC20Test(outTokens[i]).balanceOf(address(this));
            } else {
    ...
    function _safeApprove(ERC20Test token, address spender, uint amount) internal {
