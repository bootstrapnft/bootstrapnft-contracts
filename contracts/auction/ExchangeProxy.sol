...
contract ExchangeProxy is Ownable {
    ...
    returns (uint totalAmountIn)
    {
        ...
        for (uint i = 0; i < swapSequences.length; i++) {
            ...
            if (swapSequences[i].length == 1) {
                ...
                if (FirstSwapTokenIn.allowance(address(this), firstSwap.pool) < type(uint).max) {
                    FirstSwapTokenIn.approve(firstSwap.pool, type(uint).max);
                }
                ...
                if (SecondSwapTokenIn.allowance(address(this), secondSwap.pool) < type(uint).max) {
                    SecondSwapTokenIn.approve(secondSwap.pool, type(uint).max);
                }
    ...
    returns (Swap[] memory swaps, uint totalOutput)
    {
        ...
        for (uint i = 0; i < pools.length; i++) {
            ...
            tokenOut: tokenOut,
            swapAmount: bestInputAmounts[i],
            limitReturnAmount: type(uint).max,
            maxPrice: type(uint).max
            });
        }
