...
contract DSAuth is DSAuthEvents {
    ...
    modifier auth {
        require(isAuthorized(msg.sender, msg.sig), "AUTH_ERROR");
        _;
    }
...
//    }
    ...
    // use the proxy to execute calldata _data on contract _target
    function execute(address _target, bytes memory _data) public payable auth note returns (bytes memory returndata) {
...
contract ProxyRegistry {
    ...
    function build(address owner) public returns (address payable _proxy) {
        ...
        DSProxy proxy = factory.build(owner);
